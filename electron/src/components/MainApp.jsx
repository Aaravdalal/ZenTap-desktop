...
export default function MainApp() {
  ...
  return (
    <>
      <RippleCanvas />
      <div className={`global-model-container ${activeTab === 'home' ? 'visible' : 'hidden'}`}>
      ...
      <div className="main-app-shell">
        ...
        <div className="main-app-card-area">
          {activeTab === 'home' && (
            <HomeScreen {...dockProps} screenTime={screenTime} isBlocking={isBlocking} onStartZen={startZenFlow} />
          )}
          {activeTab === 'session' && (
            <SessionScreen {...dockProps} isBlocking={isBlocking} onStartZen={startZenFlow} />
          )}
          {activeTab === 'statistics' && (
            detailedStatsItem ? (
              <DetailedStatisticsScreen item={detailedStatsItem} onBack={() => setDetailedStatsItem(null)} />
            ) : (
              <StatisticsScreen
          ...
          {activeTab === 'settings' && (
            <SettingsScreen isBlocking={isBlocking} onEmergencyUnblock={() => { window.electron?.stopBlocking(); setIsBlocking(false); }} />
          )}
          {activeTab === 'profile' && (
            <ProfileScreen screenTime={screenTime} />
          )}
        </div>
